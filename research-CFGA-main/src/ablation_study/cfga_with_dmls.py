import sys
sys.path.append("./")
from about_msloss.net_res50 import FGIAnet100_metric5_ms,FGIAnet100_metric5_circle_loss_pariwise,FGIAnet100_metric5_proxy_anchor
...
import cv2
from cfga_core.grad_cam import PoolCam_yuhan_kernel_embedding_ablation_1
...
parser.add_argument("--dml_tag", type=str, help="所评估的度量学习方法的tag.")
...
for index in range(len(args.model_name)):
    ...
    print("正在加载deep metric learning微调后模型:{}".format(args.model_name[index]))
    ...
    for phase in ['test']:
        for images, labels in loaders[phase]:#一个迭代器，迭代数据集中的每一batch_size图片;迭代的返回值dataset的子类中的__getitem__()方法是如何进行重写的；
            ...
            for flip in range(1):
                ...
                grad_cam_scda = PoolCam_yuhan_kernel_embedding_ablation_1(net)
    ...
    filename=join(target_path,'layer4_1_conv1_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_1_conv3_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_2_conv3_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_2_conv1_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_1_conv2_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_2_conv2_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'layer4_conv2_weight.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'embedding_max_avg.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'scda_max_avg_norm.json')
    ...
    final_features['train']=train_data
    final_features['test']=test_data
    filename=join(target_path,'scda_norm_max_avg.json')
