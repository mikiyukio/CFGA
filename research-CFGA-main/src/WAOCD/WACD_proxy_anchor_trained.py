import sys
sys.path.append("./")
from about_msloss import dataloader_proxy_anchor
import argparse
...
import cv2
from cfga_core.grad_cam import PoolCam_yuhan_kernel_embedding_proxy_anchor
...
class Resnet50(nn.Module):
    def __init__(self, embedding_size, pretrained=False, is_norm=True, bn_freeze=False):
...
for index in range(len(args.model_name)):
    ...
    loaders = dataloader_proxy_anchor.get_dataloaders(train_paths, test_paths,train_labels,test_labels,args.img_size, 1,1,1,SCDA_flag=1)#返回值为由可迭代DataLoader对象所组成的字典
