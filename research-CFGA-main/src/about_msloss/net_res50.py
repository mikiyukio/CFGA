...
class ResNet_circle_loss(nn.Module):
    ...
    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
        ...
        self.class_fc_2 = nn.Parameter(torch.FloatTensor(100,512))
...
class ResNet_ms_loss(nn.Module):

    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_ms_loss, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
    ...
    def forward(self, x):
        if self.mode=='ms_loss':
            return self._forward_imp2(x)




class ResNet_proxy_anchor(nn.Module):

    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_proxy_anchor, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.global_max_pool = nn.AdaptiveMaxPool2d((1, 1))
        self.class_fc = nn.Linear(512 * block.expansion*2, 512)
    ...
    def forward(self, x):
        if self.mode=='proxy_anchor_loss':
            return self._forward_imp2(x)
...
class ResNet_circle_loss_pariwise(nn.Module):

    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_circle_loss_pariwise, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
class ResNet_DGCRL(nn.Module):
    ...
    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
        ...
        self.global_avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.global_max_pool = nn.AdaptiveMaxPool2d((1, 1))
        self.class_fc = nn.Linear(512 * block.expansion, 100,bias=False)
        self.class_fc_2 = nn.Linear(512 * block.expansion * 2, 100,bias=False)
        self._scale=scale
        self.mode=mode
    ...
    def _forward_imp3_1(self, x):
        ...
        x = x * self._scale
        ##################################################
        #有问题就注释掉,这是对齐曾先生的实现的修改的第一处
        # self.class_fc_2.weight.data=nn.functional.normalize(self.class_fc_2.weight,p=2, dim=1, eps=1e-12)
        # print(self.class_fc_2.weight.size())
        ##################################################
        x = self.class_fc_2(x)
...
class ResNet_AMsoftmax(nn.Module):
    ...
    def __init__(self, block, layers,mode=None,  zero_init_residual=False,
        ...
        self.class_fc_3 = nn.Linear(512, 100,bias=False)
        self.mode=mode
    ...
    def _forward_imp4(self, x):
        ...
        return out, img_f, score, self.class_fc_3.weight  # [batchsize,200]
...
def _resnet_ms_loss( block, layers,mode, **kwargs):
    model = ResNet_ms_loss(block, layers,mode, **kwargs)
    return model

def _resnet_circle_loss_pariwise( block, layers,mode, **kwargs):
    model = ResNet_circle_loss_pariwise(block, layers,mode, **kwargs)
    return model
...
def _resnet_proxy_anchor_loss( block, layers,mode, **kwargs):
    model = ResNet_proxy_anchor(block, layers,mode, **kwargs)
    return model
...
def FGIAnet100_metric5_ms(**kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_ms_loss(Bottleneck, [3, 4, 6, 3],'ms_loss',
                   **kwargs)

def FGIAnet100_metric5_circle_loss_pariwise(**kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_circle_loss_pariwise(Bottleneck, [3, 4, 6, 3],'ms_loss',
                   **kwargs)
...
def FGIAnet100_metric5_proxy_anchor(**kwargs):
    r"""ResNet-50 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet_proxy_anchor_loss(Bottleneck, [3, 4, 6, 3],'proxy_anchor_loss',
                   **kwargs)
